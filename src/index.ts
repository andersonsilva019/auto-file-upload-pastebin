...
async function main() {
  ...
  const fileStatus = readlineSync.question(
    "Digite o status do arquivo( 0=public 1=unlisted 2=private ): "
  );

  const fileExpireDate = readlineSync.question(
    "Digite o tempo de expiração do arquivo( N=never 10M=10 minutes 1H=1 hour 1D=1 day 1W=1 week 2W=2 weeks 1M=1 month 6M=6 months 1Y=1 year ): "
  );
